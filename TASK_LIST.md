...
### 1.1 项目配置
- [ ] 添加必要依赖 (ExoPlayer/Media3, Coil图片加载, DataStore设置存储)
...
- [ ] 创建数据模型
  ...
  - PlayerConfig (播放器配置)
  - AppSettings (应用设置)
- [ ] 创建本地存储管理器 (LocalStorageManager)
- [ ] 创建 DataStore 设置管理器 (SettingsRepository)
...
### 2.1 播放器组件
- [ ] 创建视频播放器组件 (VideoPlayerView) - 基于 Media3/ExoPlayer
- [ ] 创建图片显示组件 (ImageDisplayView) - 支持图片轮播
- [ ] 创建统一媒体播放器组件 (MediaPlayerView) - 根据文件类型自动切换
- [ ] 实现播放器控制逻辑 (播放、暂停、循环)

### 2.2 2x2 布局实现
- [ ] 创建 QuadPlayerLayout 组件 (4宫格布局)
- [ ] 实现各播放器独立控制
- [ ] 实现全屏切换功能 (单击某个播放器全屏)
- [ ] 实现布局自适应 (横屏/竖屏)

### 2.3 媒体文件管理
- [ ] 创建文件类型识别工具 (MediaTypeUtils)
  - 支持视频格式: mp4, mkv, avi, mov, wmv, flv
  - 支持图片格式: jpg, jpeg, png, bmp, gif, webp
- [ ] 创建文件选择器功能 (打开系统文件管理器)
- [ ] 创建媒体扫描器 (MediaScanner)
...
### 3.1 U盘监听服务
- [ ] 创建 USB 广播接收器 (UsbBroadcastReceiver)
- [ ] 创建 U盘挂载监听服务 (UsbMonitorService)
- [ ] 实现 U盘插入/拔出事件处理
- [ ] 实现 U盘路径自动检测
